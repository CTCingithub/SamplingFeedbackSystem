function result = DiscreteMapping(ts, config)
    addpath("PandaDyn\");
    addpath("utils\");
    ...
    result.A_cal=[zeros(config.dimension),eye(config.dimension); ...
        -result.K_G/result.M_d,-result.K_f/result.M_d];
    result.B_cal=[zeros(config.dimension);eye(config.dimension)];
    result.A=expm(result.A_cal*ts);
    toc
    disp("Done")
    ...
    elseif strcmpi(config.MatExpOpt, "Trunc")
        integral_part=vpaintegral( ...
            ...
            tau,0,ts,"RelTol",1e-30,"AbsTol",1e-30);
        integral_part=double(integral_part);
